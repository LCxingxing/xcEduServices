...
public class TestSearch {
    ...
    //根据id查询(根据id精准匹配,注意当id为数组时,使用termsQuery不是termQuery)
    @Test
    ...
    //MatchQuery(分词匹配搜索,会先将搜索词进行分词,并指定匹配度,80%即分三个词后,符合两个词即搜索出来)
    @Test
    ...
    public void testHighlight() throws IOException, ParseException {
        ...
        highlightBuilder.fields().add(new HighlightBuilder.Field("name"));//设置高亮域,即对name高亮显示
//        highlightBuilder.fields().add(new HighlightBuilder.Field("description"));
        ...
        //日期格式化对象
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        //解析高亮内容(高亮内容中包含原始文档和高亮后的文档,所以对高亮后的文档进行获取)
        for(SearchHit hit:searchHits){
            //文档的主键
            String id = hit.getId();
            //源文档内容
